...
class JsonMasterDataStore:
    ...
    @staticmethod
    def _load_list(path: Path, mapper: Callable[[dict[str, Any]], T]) -> List[T]:
    ...
    @staticmethod
    def _save_list(path: Path, items: Iterable[T], serializer: Callable[[T], dict[str, Any]]) -> None:
    ...
    def load_racks(self) -> List[Rack]:
        return self._load_list(self._paths.racks, Rack.from_dict)

    def save_racks(self, racks: Sequence[Rack]) -> None:
        self._save_list(self._paths.racks, racks, Rack.to_dict)

    def load_drawers(self) -> List[Drawer]:
        return self._load_list(self._paths.drawers, Drawer.from_dict)

    def save_drawers(self, drawers: Sequence[Drawer]) -> None:
        self._save_list(self._paths.drawers, drawers, Drawer.to_dict)

    def load_parts(self) -> List[Part]:
        return self._load_list(self._paths.parts, Part.from_dict)

    def save_parts(self, parts: Sequence[Part]) -> None:
        self._save_list(self._paths.parts, parts, Part.to_dict)

    def load_categories(self) -> List[Category]:
        return self._load_list(self._paths.categories, Category.from_dict)

    def save_categories(self, categories: Sequence[Category]) -> None:
        self._save_list(self._paths.categories, categories, Category.to_dict)

    def load_manufacturers(self) -> List[Manufacturer]:
        return self._load_list(self._paths.manufacturers, Manufacturer.from_dict)

    def save_manufacturers(self, manufacturers: Sequence[Manufacturer]) -> None:
        self._save_list(self._paths.manufacturers, manufacturers, Manufacturer.to_dict)

    def load_tags(self) -> List[Tag]:
        return self._load_list(self._paths.tags, Tag.from_dict)

    def save_tags(self, tags: Sequence[Tag]) -> None:
        self._save_list(self._paths.tags, tags, Tag.to_dict)

    def load_locations(self) -> List[Location]:
        return self._load_list(self._paths.locations, Location.from_dict)

    def save_locations(self, locations: Sequence[Location]) -> None:
        self._save_list(self._paths.locations, locations, Location.to_dict)
...
class JsonIndexDataStore:
    ...
    def load_parts_by_tag(self) -> List[PartsByTag]:
        return JsonMasterDataStore._load_list(self._paths.parts_by_tag(), PartsByTag.from_dict)

    def save_parts_by_tag(self, items: Sequence[PartsByTag]) -> None:
        JsonMasterDataStore._save_list(self._paths.parts_by_tag(), items, PartsByTag.to_dict)

    def load_parts_by_category(self) -> List[PartsByCategory]:
        return JsonMasterDataStore._load_list(
            self._paths.parts_by_category(), PartsByCategory.from_dict
        )

    def save_parts_by_category(self, items: Sequence[PartsByCategory]) -> None:
        JsonMasterDataStore._save_list(
            self._paths.parts_by_category(), items, PartsByCategory.to_dict
        )

    def load_parts_by_drawer(self) -> List[PartsByDrawer]:
        return JsonMasterDataStore._load_list(
            self._paths.parts_by_drawer(), PartsByDrawer.from_dict
        )

    def save_parts_by_drawer(self, items: Sequence[PartsByDrawer]) -> None:
        JsonMasterDataStore._save_list(
            self._paths.parts_by_drawer(), items, PartsByDrawer.to_dict
        )
